...
export default function SpinnerModal({
  ...
  // Handle the click
  const handleSpin = () => {
    if (!canSpin || isSpinning || userSpin) return;
 
    // 1. Start the state
    dispatch(startSpinning());
    setShowResult(false);
 
    // 2. Generate the outcome in Redux
  ...
  useEffect(() => {
    ...
    if (isSpinning && userSpin) {
      const outcome = (userSpin as any).outcome;
      ...
      setTimeout(() => {
        ...
        setShowResult(true);
        onSpinComplete?.(outcome, (userSpin as any).giftAwarded);
      }, 3000);
    }
    // If user loads page and has ALREADY spun previously
    else if (!isSpinning && userSpin && finalOutcome === null) {
      const outcome = (userSpin as any).outcome;
      setFinalOutcome(outcome);
      setShowResult(true);
    }
  }, [userSpin, isSpinning, dispatch]);
 
  const getStatusMessage = () => {
    if (userSpin) return "You've already spun!";
    if (remainingGifts === 0) return "All gifts claimed";
    if (canSpin) return "Spin to Win!";
    return `${remainingGifts} gifts left`;
  };
