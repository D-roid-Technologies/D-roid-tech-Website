...
const Leave: React.FunctionComponent = () => {
  ...
  function timeSinceAccountCreation(creationDateStr: string): {
    months: number;
    days: number;
    hours: number;
  } {
  ...
  // Use joinDate from Redux instead of hardcoded date
  // Add fallback in case joinDate is empty
  const result = joinDate
    ? timeSinceAccountCreation(joinDate)
    ...
    if (months > 6 && months < 12) {
      return (
        <p style={{ color: "#000000", marginBottom: 20 }}>
          You have 6 days of Leave available.
        </p>
      );
    } else if (months >= 12) {
      return (
      ...
      return (
        <p style={{ color: "#000000", marginBottom: 20 }}>
          You have 0 days of Leave available.
        </p>
      );
    }
  };

  // Handle case where joinDate might be empty
  if (!joinDate) {
    return (
      <div>
        <p style={{ color: "#ff0000", marginBottom: 20 }}>
          No join date found. Please contact HR to update your profile.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <p style={{ color: "#000000", marginBottom: 10 }}>
        ...
        Months, {result.days} Days and {result.hours} Hours
      </p>
      ...
      {["StartDay", "EndDay", "Month", "Year"].map((field) => (
        <input
          ...
          style={{
            ...
            fontSize: "0.875rem",
            backgroundColor: result.months < 6 ? "#F3F4F6" : "#FFFFFF",
            cursor: result.months < 6 ? "not-allowed" : "text",
          }}
        />
      ))}
      <button
