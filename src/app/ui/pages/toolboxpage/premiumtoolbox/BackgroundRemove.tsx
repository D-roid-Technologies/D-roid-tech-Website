...
const BackgroundRemove: React.FC<BackgroundRemoverProps> = ({ onClose }) => {
  ...
  const [outputFormat, setOutputFormat] = useState<"png" | "webp" | "jpeg">(
    "png"
  );
  ...
  const removeBackgroundWithRemoveBg = async (
    imageBlob: Blob
  ): Promise<string> => {
    if (!apiKey.trim()) {
    ...
    if (!response.ok) {
      if (response.status === 402) {
        throw new Error(
          "D'roid.bg: Insufficient credits. Please check your account."
        );
      } else if (response.status === 403) {
        throw new Error(
          "D'roid.bg: Invalid API key. Please check your API key."
        );
      } else if (response.status === 400) {
        throw new Error("D'roid.bg: Invalid image format or size.");
      } else {
        throw new Error(
          `D'roid.bg API error: ${response.status} - ${response.statusText}`
        );
      }
    }
    ...
    <div className="airbg-container">
      <div className="airbg-header">
        <div className="airbg-header-content">
          ...
          <p className="airbg-subtitle">
            Professional background removal using advanced AI technology
          </p>
          {apiKey && (
      ...
      <div className="airbg-features">
        <div className="airbg-feature">
          <div className="airbg-feature-icon">
            <FaTrophy />
          </div>
          <h3>Industry Leading</h3>
          <p>
            Remove.bg is the most trusted background removal service used by
            millions
          </p>
        </div>
        <div className="airbg-feature">
