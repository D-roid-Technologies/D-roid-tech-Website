...
const PricingCalculator: React.FC = () => {
    ...
    return (
        <div style={styles.container}>
                ...
                <div key={service.category} style={styles.category}>
                    ...
                    {service.options.map((option) => (
                        <div key={option.name} style={styles.serviceRow}>
                            <label style={styles.label}>{option.name} (${option.price})</label>
                            <input
                                type="number"
                                min={0}
                                placeholder="Qty"
                                value={selectedServices[option.name] || ''}
                                onChange={(e) => handleQuantityChange(option.name, parseInt(e.target.value) || 0)}
                                style={styles.input}
                            />
                        </div>
                    ))}
            ...
            {total !== null && (
                <div style={{ marginTop: "20px", fontSize: "16px", textAlign: "center", color: "#000000" }}>
